...
static void dump_to_file(unsigned long buf, size_t size, loff_t *offset) 
{
	...
	// Permanent path
	//sprintf(file_path, "/lkmc/dump/%lx_%lu", (unsigned long) buf, epoch_counter);
...
static int fh_init(void)
{
	...
	if (err)
		return err;
	pr_info("Ftrace Hook: Module loaded\n");
...
static void fh_exit(void)
{
	...
	pr_info("Ftrace Hook: Module unloaded\n");
}
module_exit(fh_exit);


MODULE_DESCRIPTION("Hooking module");
/* 
 * Modified by Matteo Giordano @ Polimi
*/
